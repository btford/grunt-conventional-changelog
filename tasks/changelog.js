...
var parseRawCommit = function(raw) {
    ...
    if (!match) {
        match = msg.subject.match(PATTERN2);
    }

    if (!match || !match[1] || !match[4]) {
        warn('Incorrect message: %s %s', msg.hash, msg.subject, match);
        return null;
    }
...
var Writer = function(stream, repoType, repoUrl) {
    var linkToIssue = function(issue) {
        return util.format(LINK_ISSUE, issue, repoUrl, issue);
    };

    var linkToCommit = function(hash) {
        if (repoType != 2) {
            return util.format(LINK_COMMIT, hash.substr(0, 8), repoUrl, hash);

        } else {
            return util.format(LINK_COMMIT_BITBUCKET, hash.substr(0, 8), repoUrl, hash);
...
// PUBLIC API
exports.generate = function(repoType, repoUrl, version) {
    var buffer = {
        data: '',
        write: function(str) {
            this.data += str;
    ...
    var writer = new Writer(buffer, repoType, repoUrl);
