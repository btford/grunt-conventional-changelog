...
var Writer = function(stream, githubRepo) {
  ...
  this.section = function(title, section, allNested) {
...
var writeChangelog = function(writer, commits, version, allNested) {
  var sections = {
    fix: {},
    feat: {},
    breaks: {}
  ...
  writer.section('Bug Fixes', sections.fix, allNested);
  writer.section('Features', sections.feat, allNested);
  writer.section('Breaking Changes', sections.breaks, allNested);
};
...
// PUBLIC API
exports.generate = function(githubRepo, version, allNested) {
  var buffer = {
    data: '',
    write: function(str) {
      this.data += str;
  ...
  return getPreviousTag().then(function(tag) {
    return readGitLog('^fix|^feat|BREAKING', tag).then(function(commits) {
      writeChangelog(writer, commits, version, allNested);
      return buffer.data;
    });
  });
};
